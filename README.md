...
- One-hour blocks from 06:00 to 23:00
...
- Dates are **23 Sep 2026 to 7 Oct 2026 inclusive**. Exact flight arrival and departure times were not supplied, so Day 1 and Day 15 use flexible airport buffers.
...
- Airbnb links are search links, not endorsements of individual listings.
...
1. **Vancouver:** stay downtown for the first three nights; use a Vancouver or Richmond / YVR hotel for the final night depending on flight time.
...
6. **McArthurGlen:** use it on arrival only if energy allows, and again on departure only if the flight leaves a real airport buffer.
...
- Airline baggage allowance before the outlet visit

## Files

- `index.html` - interactive itinerary board
- `data/itinerary.json` - machine-readable trip data
- `SOURCES.md` - source ledger and verification notes
- `scripts/build.py` - original deterministic data generator
- `scripts/update_plan.py` - updated plan generator for the Vancouver + Canmore version
